    ...
    return (
      <div className="flex flex-col gap-8 lg:gap-[48px]">
        ...
        <div className="flex flex-col gap-4">
          <Link href="/app/groups" className="text-[15px] text-slate-gray hover:text-ink-black transition-colors w-fit">
            &larr; Back to Groups
          </Link>
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mt-2">
            <div>
              <h1 
                className="text-[44px] leading-[1.3] text-ink-black font-serif font-normal flex items-center gap-4"
                style={{ letterSpacing: '-0.66px' }}
              >
                Group {group.groupNumber}: {group.name}
              </h1>
              <p className="text-[17px] text-slate-gray max-w-[600px] leading-[1.35] mt-2">
                Centre: {group.centre.name} {group.centre.branch?.name ? `(${group.centre.branch.name})` : ''}
              </p>
            </div>
            <div className="flex items-center gap-4 bg-paper-white px-[20px] py-[12px] rounded-full shadow-subtle-3 border border-[#ececec]">
              <div className="flex flex-col">
                <span className="text-[12px] text-slate-gray uppercase font-medium">Status</span>
                <span className={`text-[15px] font-medium ${group.status === 'ACTIVE' ? 'text-green-600' : 'text-slate-gray'}`}>
                  {group.status}
                </span>
              </div>
              <div className="w-[1px] h-[32px] bg-[#ececec]"></div>
              <div className="flex flex-col">
                <span className="text-[12px] text-slate-gray uppercase font-medium">Schedule</span>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-border/40">
                      ...
                      <th className="pb-4 font-sans text-[15px] text-slate-gray font-normal">Contact</th>
                      <th className="pb-4 font-sans text-[15px] text-slate-gray font-normal">Joined</th>
                      <th className="pb-4 font-sans text-[15px] text-slate-gray font-normal">Role</th>
                      <th className="pb-4 font-sans text-[15px] text-slate-gray font-normal">Status</th>
                      <th className="pb-4 font-sans text-[15px] text-slate-gray font-normal">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {group.memberships.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="text-center py-12 text-[15px] text-slate-gray">
