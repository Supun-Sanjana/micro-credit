...
export default function LoanDetailPage() {
  ...
  if (!loan || loan.error) {
    return <p className="p-8 text-slate-500">{loan?.error || "Loading loan…"}</p>
  }
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">{loan.member?.name} · {loan.member?.memberNumber}</p>
        </div>
        <span className="rounded-full bg-slate-50 px-4 py-2 text-sm">{loan.status}</span>
      </header>

      {notice && <p className="rounded-lg bg-slate-50 p-3">{notice}</p>}
      ...
      <section className="rounded-2xl bg-slate-50 p-5">
        ...
        <p className="mb-4 mt-1 text-sm text-slate-500">These preserve the original loan and payment history.</p>
        <div className="flex flex-wrap gap-3">
          
          <Dialog open={openDialog === 'TOP_UP'} onOpenChange={(o) => setOpenDialog(o ? 'TOP_UP' : null)}>
            <DialogTrigger disabled={busy} className="rounded-full border bg-white px-4 py-2 text-sm font-medium disabled:opacity-50 hover:bg-slate-50 transition-colors">
              Top-up / Refinance
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
              ...
              <form onSubmit={(e) => {
                ...
                <div>
                  <label className="block text-sm text-slate-500 mb-1">New Loan Amount</label>
                  <input required type="number" name="amount" min={Number(loan.outstanding) + 1} step="0.01" className="w-full bg-white border border-[#ececec] rounded-[16px] px-[16px] py-[14px]" placeholder={`Must exceed ${loan.outstanding}`} />
                </div>
                <div>
                  <label className="block text-sm text-slate-500 mb-1">Reason</label>
                  <textarea name="reason" className="w-full bg-white border border-[#ececec] rounded-[16px] px-[16px] py-[14px] min-h-[100px]" placeholder="Reason for top-up"></textarea>
                </div>
                <DialogFooter>
                  <button type="submit" disabled={busy} className="bg-navy-900 text-white rounded-full px-6 py-2.5 text-sm font-medium hover:bg-navy-900/90 transition-colors">
                    Confirm Top-up
                  </button>
                </DialogFooter>
              </form>
          ...
          <Dialog open={openDialog === 'WRITE_OFF'} onOpenChange={(o) => setOpenDialog(o ? 'WRITE_OFF' : null)}>
            ...
            <DialogContent>
              ...
              <form onSubmit={(e) => {
                ...
                <div>
                  <label className="block text-sm text-slate-500 mb-1">Write-off Reason <span className="text-brand-700">*</span></label>
                  <textarea required name="reason" className="w-full bg-white border border-[#ececec] rounded-[16px] px-[16px] py-[14px] min-h-[100px]" placeholder="Enter reason for write-off..."></textarea>
                </div>
                <DialogFooter>
                  <button type="submit" disabled={busy} className="bg-brand-700 text-white rounded-full px-6 py-2.5 text-sm font-medium hover:bg-brand-700/90 transition-colors">
                    Confirm Write-off
                  </button>
                </DialogFooter>
              </form>
              ...
              <form onSubmit={(e) => {
                ...
                <div className="bg-[#f8f8f8] rounded-[16px] p-4 text-sm flex gap-4 border border-[#ececec]">
                  <div className="flex-1">
                    <span className="text-slate-500 block text-xs mb-1">Total Written Off</span>
                    <span className="font-medium">LKR {totalWrittenOff.toLocaleString()}</span>
                  </div>
                  <div className="flex-1">
                    <span className="text-slate-500 block text-xs mb-1">Total Recovered</span>
                    <span className="font-medium text-[#059669]">LKR {totalRecovered.toLocaleString()}</span>
                  </div>
                </div>
                <div>
                  <label className="block text-sm text-slate-500 mb-1">Amount <span className="text-brand-700">*</span></label>
                  <input required type="number" name="amount" min="0.01" step="0.01" className="w-full bg-white border border-[#ececec] rounded-[16px] px-[16px] py-[14px]" placeholder="0.00" />
                </div>
                <div>
                  <label className="block text-sm text-slate-500 mb-1">Date</label>
                  <input required type="date" name="date" defaultValue={new Date().toISOString().split('T')[0]} className="w-full bg-white border border-[#ececec] rounded-[16px] px-[16px] py-[14px]" />
                </div>
                <div>
                  <label className="block text-sm text-slate-500 mb-1">Note (optional)</label>
                  <textarea name="note" className="w-full bg-white border border-[#ececec] rounded-[16px] px-[16px] py-[14px] min-h-[80px]" placeholder="Additional details..."></textarea>
                </div>
                <DialogFooter>
                  <button type="submit" disabled={busy} className="bg-navy-900 text-white rounded-full px-6 py-2.5 text-sm font-medium hover:bg-navy-900/90 transition-colors">
                    Save Recovery
        ...
        <div className="mt-6 pt-5 border-t border-[#ececec]">
          <p className="mb-3 text-sm text-slate-500">Restructuring requires a reviewed replacement schedule.</p>
          <Link href={`/app/loans/${id}/restructure`} className="inline-flex items-center justify-center rounded-full border border-[#ececec] bg-white text-navy-900 px-6 py-2.5 text-sm font-medium hover:bg-[#fafafa] transition-colors shadow-subtle-1">
            Restructure Loan
          </Link>
        </div>
      </section>
        ...
        <div className="divide-y border-t mt-3">
          {loan.repayments?.map((r: any) => (
            <div className="flex justify-between py-3 text-sm" key={r.id}>
              <span className="text-slate-500">{format(new Date(r.paidDate), "PP")} · {r.transactionType} {r.method ? `· ${r.method}` : ''}</span>
              <b className="text-navy-900">LKR {Number(r.amount).toLocaleString()}</b>
            </div>
          ))}
          {(!loan.repayments || loan.repayments.length === 0) && (
            <p className="py-4 text-slate-500 text-sm">No repayments recorded.</p>
          )}
        </div>
      </section>
    </div>
