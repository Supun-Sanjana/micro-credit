      ...
      <div className="lg:col-span-4 h-fit">
        <div className="bg-paper-white rounded-[20px] shadow-subtle-3 p-[32px]">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[15px] text-ink-black font-sans ml-1">Type</label>
              <select 
                value={formData.type}
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-[#ececec] pt-5">
              <div className="flex flex-col gap-2">
                <label className="text-[15px] text-ink-black font-sans ml-1">Interest Rate (%)</label>
                <input 
                  required
                  type="number"
                  step="0.1"
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[15px] text-ink-black font-sans ml-1">Min. Balance</label>
                <input 
                  required
                  type="number"
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-border/40">
                  <th className="pb-4 font-sans text-[15px] text-slate-gray font-normal">Name & Code</th>
                  <th className="pb-4 font-sans text-[15px] text-slate-gray font-normal">Type</th>
                  <th className="pb-4 font-sans text-[15px] text-slate-gray font-normal">Interest</th>
                  <th className="pb-4 font-sans text-[15px] text-slate-gray font-normal">Min. Balance</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  initialProducts.map((product) => (
                    <tr key={product.id} className="border-b border-border/40 last:border-0">
                      <td className="py-5 pr-4">
                        <div className="text-[16px] font-sans font-medium text-ink-black">{product.name}</div>
                        <div className="text-[14px] mt-1 text-slate-gray">{product.code}</div>
                      </td>
                      <td className="py-5 pr-4">
                      ...
                      <td className="py-5 pr-4 text-[15px] font-sans text-ink-black">
